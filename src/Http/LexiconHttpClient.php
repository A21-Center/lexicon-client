...
class LexiconHttpClient
{
    ...
    /**
     * Push a single extracted entity to the Integration translate endpoint.
     *
     * @param  array<string, mixed>  $payload
     * @return array<string, mixed>
     */
    ...
    {
        return $this->request()
            ->post($this->endpoint('/integrations/translate'), $payload)
            ->throw()
            ->json('data');
    }

    /**
     * Report exact-literal key usage for an application (Used / Unused).
     *
     * @param  array{
     *     application: string,
     *     project_code?: string,
     *     replace_unmentioned?: bool,
     *     results: list<array{full_key: string, status: string, evidence?: list<array{path: string, line?: int}>}>
     * }  $payload
     * @return array<string, mixed>
     */
    ...
    {
        return $this->request(timeout: (int) config('lexicon.http.import_timeout', 180))
            ->post($this->endpoint('/integrations/usage-report'), $payload)
            ->throw()
            ->json('data');
    }
