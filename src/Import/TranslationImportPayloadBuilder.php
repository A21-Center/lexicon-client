...
class TranslationImportPayloadBuilder
{
    ...
    public function build(
        ...
        foreach ($files as $file) {
            ...
            $folderCode = $this->folderCodeFromRelativePath($file['relative_path']);
            if ($folderCode !== null) {
                $folders[$folderCode] = true;
            }
    ...
    private function folderCodeFromRelativePath(string $relativePath): ?string
    {
        ...
        $segment = explode('/', $normalized)[0] ?? '';
        $code = strtolower(preg_replace('/\s+/', '_', $segment) ?? $segment);
