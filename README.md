...
Laravel package to connect **any** client application to a central **Lexicon**
translation server.
...
- **receive UGC translations** from Lexicon into a local `entity_translations`
  table via `POST /api/localization/sync` (Google MT / Gallery),
- do all of it through Artisan commands, with secrets kept server-side.

- [Requirements](#requirements)
- [Installation](#installation)
- [Configuration](#configuration)
- [Environment variables](#environment-variables)
- [UGC sync (entity_translations)](#ugc-sync-entity_translations)
...
For Gallery / UGC Google MT sync (consumer table + webhook):

```bash
php artisan lexicon:install-ugc-sync --migrate
```

See [docs/ugc-entity-translations-sync.md](docs/ugc-entity-translations-sync.md).
...
# optional — Lexicon → app UGC sync (Gallery)
...
`LEXICON_SYNC_SECRET` must match `integration_clients.sync_secret` on Lexicon,
and Lexicon `sync_url` must point to `https://your-api/api/localization/sync`.

## UGC sync (entity_translations)

Dual-store model:

| Store | Role |
|-------|------|
| Lexicon DB | Edit / Google MT / approve |
| This app DB | Synced copy for Gallery (approved-only) |

```bash
php artisan lexicon:install-ugc-sync --migrate
```

This publishes the `entity_translations` migration, registers
`POST /api/localization/sync` when `LEXICON_UGC_SYNC_ENABLED=true`, and documents
the two secrets (inbound client secret vs outbound sync secret).

Full guide: [docs/ugc-entity-translations-sync.md](docs/ugc-entity-translations-sync.md).
...
| `php artisan lexicon:install-ugc-sync` | Publish `entity_translations` migration + enable Lexicon→app sync webhook. |
...
| `php artisan lexicon:scan-usage` | Scan exact translation key literals in client code and report Used/Unused to Lexicon. |

### Key usage scan (exact literals only)

V1 marks a catalog key **Used** only when an exact string literal appears in code
(`__('age_artist')`, `@lang('…')`, `t('…')`, …). Dynamic concatenations like
`__('artist.'.$id)` are ignored and stay **Unused** unless the same literal is
found elsewhere.

```bash
# Dry-run against local lang/ + resources/app/src
php artisan lexicon:scan-usage --application=studio --dry-run

# Post report to Lexicon (marks unmentioned Lexicon keys Unused for that app)
php artisan lexicon:scan-usage --application=studio
php artisan lexicon:scan-usage --application=gallery --path=resources --lang=lang
```

Options: `--application`, `--path=*`, `--lang=*`, `--dry-run`,
`--no-replace-unmentioned`.
...
**Google MT:** Lexicon generates and accepts Google MT suggestions for **all**
layers (`content`, `interface`, `template`, `database`). The Lexicon UI review
workflow labels (`draft` / `needs_review`) are emphasized for `content`
projects; UGC / Gallery entity sync (`needs_review=true`) remains the consumer
path for CMS-style content.
...
- Keep `LEXICON_CLIENT_SECRET` and `LEXICON_SYNC_SECRET` server-side only (`.env`),
  never in `lexicon.json` or any frontend code.
- Frontends must call **your** backend, which then calls Lexicon with the client secret.
- Lexicon calls **your** `/api/localization/sync` with the sync secret (outbound webhook).
